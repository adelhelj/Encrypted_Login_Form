...
func SignupHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // Inserting email, password, and activeUser = false into users table
    _, err = db.Exec("INSERT INTO users(email, password, activeUser) VALUES($1, $2, $3)", user.Email, user.Password, false)
    if err != nil {
...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
    ...
    //saving value of password field into correctPw for given email
    err = db.QueryRow("SELECT password FROM users WHERE email = $1", user.Email).Scan(&correctPw)
    if err != nil {
    ...
    // if the user's entered password and the password in the db for that email don't match, error
